...
\begin{description}
\item[\refstepcounter{acnum} \actheacnum \label{acHardware}:] Increase or decrease in the number of data centers.
\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] Changes in the total power consumption.
\item[\refstepcounter{acnum} \actheacnum \label{acParams}:] Changes in the distance between the power station and the data centers.
\item[\refstepcounter{acnum} \actheacnum \label{acVerify}:] Changes in the maximum power consumption per data center.
\item[\refstepcounter{acnum} \actheacnum \label{acOutput}:] Changes in the power loss due to distance.
...
\item[\refstepcounter{acnum} \actheacnum \label{acEnergy}:] Use more descriptive variable names to improve readability.
\item[\refstepcounter{acnum} \actheacnum \label{acControl}:] Add input validation to ensure that the user enters a valid number for the number of data centers.
...
\item[\refstepcounter{acnum} \actheacnum \label{ac2}:] Add error handling for cases where the number of data centers is too large or too small.
\item[\refstepcounter{acnum} \actheacnum \label{ac3}:] Add a function to calculate the power loss due to distance based on a formula rather than a constant value.
...
\begin{description}
\item[\refstepcounter{ucnum} \uctheucnum \label{ucIO}:] Input devices, only accept csv file, other type not looks like acceptable.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucInput}:] Remove the check for valid distance values between 0 and 2000.
...
\acref{acInput} & \mref{mHH}\\ %\mref{mInput}\\
